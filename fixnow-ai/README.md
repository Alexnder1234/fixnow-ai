...
## 💰 Comisión y plan Premium (ahora reales en la app, no solo en la presentación)

- **Comisión del 12%**: al pagar un ticket, el recibo del cliente y las estadísticas del técnico muestran el desglose real — cuánto se queda FixNow AI y cuánto recibe el técnico neto.
- **Plan Premium (RD$500/mes, simulado)**: dentro de "Mi perfil", un técnico puede suscribirse — mientras esté activo, aparece primero en los resultados de búsqueda de sus categorías y muestra una insignia ★ PREMIUM en su tarjeta.

### Si ya tenías la base de datos creada antes de este cambio
Corre también `src/db/migracion_premium.sql` una vez en la consola Query de Railway. Si vas a crear la base de datos desde cero, no hace falta: `schema.sql` ya la incluye.
